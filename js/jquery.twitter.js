...
;(function($, undefined){
	...
	String.prototype.format = function() {
		...
		while (l--) {
			var re = RegExp('%'+l, 'g');
			formatted = formatted.replace(re, args[l]);
		}
		...
		$.getJSON('//api.%01/statuses/user_timeline.json?count=%1&include_entities=1&exclude_replies=%2&screen_name=%3&callback=?'.format(twitterDomain, count, configs.exclude_replies, usr), function(data){
			$.each(data, function(i, tweetData) {
				...
				if (configs.timeSpan) {
					...
					time = configs.time_format.format(
						dateTweet.getDate(),
						configs.i18n_months[dateTweet.getMonth()],
						dateTweet.getFullYear(),
						dateTweet.getHours() + 'h' + (minute ? (minute<10?'0':'') + minute : '')
					);
				};
